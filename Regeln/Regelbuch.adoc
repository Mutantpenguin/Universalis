...
Ein gegnerisches Modell, welches sich zum Beginn der eigenen Initiativephase innerhalb des Gefahrenbereichs am nächsten zum eigenen Modell aufhält, wird „Unmittelbare Bedrohung" genannt.
...
|Flug
a|
* Ignoriert <<Passierbarkeit>> von <<Geländeteile,Geländeteilen>>.
...
3+<|
Mit dieser Aktion kann sich ein Modell aus einem <<Nahkampf>> lösen, muss vorher aber den Test für eine <<unmittelbare Bedrohung>> durchführen.
...
Jedes Modell kann ohne spezielle Ausrüstung oder Eigenschaften auf oder herunter von <<Geländeteile,Geländeteilen>> bis zu seinem doppelten <<Größenprofil>> klettern.
...
Geländeteile gehören in der Regel einem <<Geländetypen,Geländetyp>> an.
...
Sie beinhalten Informationen zu <<Passierbarkeit>>, <<Deckung>>, <<Sicht>> und eventuellen weiteren Sonderregeln.
