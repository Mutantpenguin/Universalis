...
Weiterhin <<Drohnen ohne Operator,verfallen>> eventuell zugeordnete Drohnen automatisch in Panik.
...
Sie haben keinen eigenen Willen und die Steuerung erfolgt ausschließlich durch den <<Operatoren,Operator>>.
...
(((Operatoren)))
Unter Operatoren versteht man alle Modelle denen eine Drohne zugeordnet werden kann.
...
* Ein Prospektor verfügt über 3 "Lenk-Module" für seinen Anzug und kann damit 3 zugeordnete Drohnen haben.
====

=== Zuordnung

(((Drohnen zuordnen)))
Drohnen können und müssen immer genau einem Operator zugeordnet sein.
Dies geschieht <<Vorbereiten der Gruppe,vor dem Spiel>> und kann währenddessen nicht geändert werden.

=== Drohnen ohne Operator

(((Drohnen ohne Operator)))
Drohnen, deren Operator nicht mehr auf dem Spielfeld präsent ist oder der <<Eliminiert,eliminiert>> wurde, erhalten einen Panikmarker (siehe <<Panik>>).
...
Die einzige Besonderheit besteht darin, dass der Operator zum Beginn einer Runde seine eigenen AP ausgeben kann um eventuelle Schock-Token von seinen ihm zugeordneten Drohnen zu entfernen.

=== Eliminierung

Sobald eine Drohne <<Eliminiert,eliminiert>> wurde, kommt es zu einer Rückkopplung auf den Operator, welche ihm Schaden zufügen kann.
...
Kräfte verfügen über die Merkmale AP-Kosten, Attribut, Ziel, Reichweite, TP-Verlust und Dauer.
