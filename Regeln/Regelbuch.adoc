...
(((Wahrnehmungsbereich)))
Der Wahrnehmungsbereich umfasst alles um ein Modell in <<Sichtlinie>> bis zu einer Distanz die wie folgt berechnet wird:
...
3+<a|* Das Modell springt entsprechend seiner GK in cm weit wenn es gestanden hat.
...
3+<a|* Aufklärungsunterstützung bezieht sich immer auf ein konkretes eigenes Modell, welches versucht aufzuklären.
...
3+<a|Eine eigene beliebige "dauerhafte" Kraft die gerade im Spiel ist wird beendet.
