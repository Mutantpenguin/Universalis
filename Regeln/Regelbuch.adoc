...
Sobald ein Modell ausgeschaltet wurde, muss für jedes andere Modell seiner Gruppe, in dessen <<Wahrnehmungsbereich>> es sich befindet, getestet werden, ob es in <<Panik>> verfällt.
...
|andere ausgeschaltete befreundete Modelle
|-2
...
3+<a|[WARNING]
====
* Kann durchgeführt werden, wenn ein feindliches Modell im Wahrnehmungsbereich eine Kraft einsetzt.
====
